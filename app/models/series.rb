    ...
    work_list.each_with_index do |work_id, index|
      ...
      if work
        if work.position == count && work.parent_id == self.id
          ...
          Rails.logger.debug "work already exists, updating #{work.title} with position #{count} and parent_id #{self.id}"
          work.update!(position: count, parent_id: self.id)
        end
      else
        Rails.logger.debug "work does not yet exist, creating ao3/works/#{work_id} in position #{count} and parent_id #{self.id}"
