...
module Scrub
  def self.agent
    @agent ||= Mechanize.new do |a|
      a.log = Logger.new(Rails.root.join('log/mechanize.log').to_s)
      a.open_timeout = 10
      a.read_timeout = 10
    end
  end
  ...
  def self.regularize_body(html)
    ...
    nodeset.to_xhtml(indent_text: '', indent: 0).delete("\n")
  end

  def self.strip_html(html) = Sanitize.fragment(html, elements: 'hr').strip.gsub(/ *<hr> */, '; ')

  def self.sanitize_and_strip(html) = strip_html(sanitize_html(html))

  def self.remove_extra_formatting(html)
    Sanitize.clean(
      html,
      elements: %w[a b big blockquote br center
  ...
  def self.sanitize_html(html)
    ...
    # remove empty divs
    html.gsub!(%r{<div>\s*</div>}, '')
    html = Scrub.remove_surrounding(html)

    # remove anchor links
    html.gsub!('<a></a>', '')
    # alright drives me batty
    html.gsub!(/([Aa])lright/) { |_s| "#{::Regexp.last_match(1)}ll right" }
    # charka => chakra
    html.gsub!(/([Cc])harka/) { |_s| "#{::Regexp.last_match(1)}hakra" }
    # chocked => choked
    html.gsub!(/([Cc])hocked/) { |_s| "#{::Regexp.last_match(1)}hocked" }
    # remove back_to_back bold and italic
    html.gsub!(%r{</b>([_., -]*)<b>}) { |_s| ::Regexp.last_match(1) }
    html.gsub!(%r{</i>([_., -]*)<i>}) { |_s| ::Regexp.last_match(1) }
    # remove empty bold and italic and underline
    html.gsub!('<u></u>') { |_s| ::Regexp.last_match(1) }
    html.gsub!('<i></i>') { |_s| ::Regexp.last_match(1) }
    html.gsub!('<b></b>') { |_s| ::Regexp.last_match(1) }
    ...
    html.gsub!(%r{<div><br ?/?>}, '<div>')
    html.gsub!(%r{<br ?/?></div>}, '</div>')
    ...
    empty_paragraphs = html.scan(%r{<p>\s*</p>}).count
    if empty_paragraphs.positive? && total_paragraphs / empty_paragraphs > 5
      html.gsub!(%r{<p>\s*</p>}, '<hr>')
    else
      html.gsub!(%r{<p>\s*</p>}, '')
    end
    ...
    html.gsub!(/<p>\s*<hr>/, '<hr>')
    html.gsub!(%r{<hr>\s*</p>}, '<hr>')
    # remove stray divs around sections
    html.gsub!(%r{<div>\s*<hr ?/?>}, '<hr>')
    html.gsub!(%r{<hr ?/?>\s*</div>}, '<hr>')

    # condense multiple sections
    html.gsub!(%r{(\s*<hr ?/?>\s*){2,}}, '<hr>')
  ...
  def self.fetch_html(url)
    ...
    if url.match?('archiveofourown.org')
      ...
      unless content.links.third.text.match(auth[:username])
        ...
        Scrub.agent.submit(form, form.buttons.first)
        content = Scrub.agent.get(url)
      end
    else
    ...
    content.body.force_encoding(Scrub.agent.page.encoding)
  end
      ...
      if work
        if work.position == count && work.parent_id == parent.id
          ...
          Rails.logger.debug do
            "work already exists, updating #{work.title} with position #{count} and parent_id #{parent.id}"
          end
        ...
        Rails.logger.debug do
          "work does not yet exist, creating ao3/works/#{work_id} in position #{count} and parent_id #{parent.id}"
        end
