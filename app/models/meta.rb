...
module Meta
  ...
  def book_doc = type == 'Book' ? Nokogiri::HTML(parts.first.raw_html) : doc
  ...
  def chapter_title
    if ao3?
      ...
      new = begin
        doc.search('option[@selected="selected"]').children[0].text.match(/\d+\. (.*)/)
      rescue StandardError
        nil
      end
      old = begin
        doc.search('option[@selected="selected"]').children[1].text.match(/\d+\. (.*)/)
      rescue StandardError
        nil
      end
  ...
  def work_summary
    if wp?
      ...
      begin
        if doc.css('.series dt')[3].text == 'Description:'
          return doc.css('.series dd')[3].css('blockquote').children.to_html
        end

        doc.css('.series dd')[4].css('blockquote').children.to_html if doc.css('.series dt')[4].text == 'Description:'
      rescue StandardError
        ''
      end
  ...
  def note_tags = inferred_tags.without(TT).without(FI).join(', ')
