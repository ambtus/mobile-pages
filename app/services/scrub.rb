# frozen_string_literal: true

module Scrub
  class << self
    def is_html?(html)
      ActionController::Base.helpers.strip_tags(html) != html
    end

    def make_safe(string)
      return '' if string.blank?

      # RubyPants turns quotes into smart quotes
      # so they don't mess up the epub command
      smart = RubyPants.new(string)
      # escape javascript takes any leftover quotes and escapes them
      ActionController::Base.helpers.escape_javascript(smart)
    end

    # regularize imported html to fix garbled encodings
    #    clean up whitespace and remove javascript
    #    returns what's inside the <body> elements
    #    to be saved as raw_html (raw == pre-sanitized)
    #    called before saving to raw_html file, and nowhere else
    def regularize_body(html)
      begin
        return '' if html.blank?
      rescue ArgumentError
        html = html.encode('UTF-8', 'windows-1252')
      end
      replacements = [
        ['&#13;', ''],
        ['&#151;', '&#8212;'],
        ['&#146;', '&#8217;'],
        ['&#145;', '&#8216;'],
        ['&#148;', '&#8221;'],
        ['&#147;', '&#8220;'],
        ['&nbsp;', ' '],
        ['Â<A0>', ' '],
        ['â€”', '—'],
        ['â€“', '—'],
        ['â€¦', '…'],
        ['Ã©', 'é'],
        ['â€œ', '“'],
        ['â€˜', '‘'],
        ['â€™', '’'],
        ['â€', '”'],
        [' ', ' '],
        ['<o:p>', '<p>'],
        ['</o:p>', '</p>']
      ]
      begin
        replacements.each do |replace|
          html = html.gsub(replace.first, replace.last)
        end
      rescue Encoding::CompatibilityError
        d = CharlockHolmes::EncodingDetector.detect(html)
        html = html.encode('UTF-8', d[:encoding], invalid: :replace, replace: '•')
        replacements.each do |replace|
          html = html.gsub(replace.first, replace.last)
        end
      end
      html.gsub!(/\s+/, ' ')
      return html unless Scrub.is_html?(html)

      doc = Nokogiri::HTML(html)
      body = doc.xpath('//body').first
      body.traverse do |node|
        case node.name
        when 'script'
          node.children.each(&:remove)
          node.remove
        end
      end
      nodeset = body.children
      nodeset.each do |top_node|
        nodeset.delete(top_node) if top_node.is_a?(Nokogiri::XML::Text) && top_node.blank?
      end
      nodeset.to_xhtml(indent_text: '', indent: 0).delete("\n")
    end

    def strip_html(html) = Sanitize.fragment(html, elements: 'hr').strip.gsub(/ *<hr> */, '; ')

    def sanitize_and_strip(html) = strip_html(sanitize_html(html))

    def remove_extra_formatting(html)
      return html unless Scrub.is_html?(html)

      Sanitize.clean(
        html,
        elements: %w[a b big blockquote br center
                     div dt em i h1 h2 h3 h4
                     h5 h6 hr img li p small
                     strike strong sub sup u],
      attributes: { 'a' => ['href'],
                    'img' => %w[align alt height src title width] }
      )
    end

    # sanitize
    def sanitize_html(html)
      return '' if html.blank?
      return html.squish unless Scrub.is_html?(html)

      html = Scrub.remove_extra_formatting(html)
      # remove empty divs
      html.gsub!(%r{<div>\s*</div>}, '')
      html = Scrub.remove_surrounding(html)

      # remove anchor links
      html.gsub!('<a></a>', '')
      # alright drives me batty
      html.gsub!(/([Aa])lright/) { |_s| "#{::Regexp.last_match(1)}ll right" }
      # charka => chakra
      html.gsub!(/([Cc])harka/) { |_s| "#{::Regexp.last_match(1)}hakra" }
      # chocked => choked
      html.gsub!(/([Cc])hocked/) { |_s| "#{::Regexp.last_match(1)}hocked" }
      # remove back_to_back bold and italic
      html.gsub!(%r{</b>([_., -]*)<b>}) { |_s| ::Regexp.last_match(1) }
      html.gsub!(%r{</i>([_., -]*)<i>}) { |_s| ::Regexp.last_match(1) }
      # remove empty bold and italic and underline
      html.gsub!('<u></u>') { |_s| ::Regexp.last_match(1) }
      html.gsub!('<i></i>') { |_s| ::Regexp.last_match(1) }
      html.gsub!('<b></b>') { |_s| ::Regexp.last_match(1) }
      # fix weird quotes being used for curly quotes
      html.tr!('`', '‘')
      html.tr!('´', '’')

      # extra breaks inside paragraphs
      html.gsub!(%r{<p><br ?/?>}, '<p>')
      html.gsub!(%r{<br ?/?></p>}, '</p>')
      # extra breaks inside divs
      html.gsub!(%r{<div><br ?/?>}, '<div>')
      html.gsub!(%r{<br ?/?></div>}, '</div>')
      # more than two breaks are probably a section
      html.gsub!(%r{(<br ?/?>){3,}}, '<hr>')
      # when sections are taken care of, multiple breaks are a paragraph
      html.gsub!(%r{(<br ?/?>?){2,}}, '<p>')

      # a few left-over empty paragraphs are probably a section
      # but too many are just extraneous whitespace
      total_paragraphs = html.scan('<p>').count
      empty_paragraphs = html.scan(%r{<p>\s*</p>}).count
      if empty_paragraphs.positive? && total_paragraphs / empty_paragraphs > 5
        html.gsub!(%r{<p>\s*</p>}, '<hr>')
      else
        html.gsub!(%r{<p>\s*</p>}, '')
      end

      # common section designation
      html.gsub!(/_{5,}/, '<hr>')
      # less common section designation
      html.gsub!(/~{10,}/, '<hr>')
      html.gsub!(/\*{10,}/, '<hr>')
      # remove stray paragraphs around sections
      html.gsub!(/<p>\s*<hr>/, '<hr>')
      html.gsub!(%r{<hr>\s*</p>}, '<hr>')
      # remove stray divs around sections
      html.gsub!(%r{<div>\s*<hr ?/?>}, '<hr>')
      html.gsub!(%r{<hr ?/?>\s*</div>}, '<hr>')

      # condense multiple sections
      html.gsub!(%r{(\s*<hr ?/?>\s*){2,}}, '<hr>')
      # remove beginning section
      html.gsub!(%r{^<hr ?/?>}, '')
      # remove end section
      html.gsub!(%r{<hr ?/?>$}, '')

      # style sections
      html.gsub!('<hr>', '<hr width="80%"/>')

      Scrub.remove_surrounding(html)
    end

    def remove_surrounding(html)
      return '' if html.blank?
      return html unless Scrub.is_html?(html)

      doc = Nokogiri::HTML(html)
      body = doc.xpath('//body').first
      nodeset = body.children
      while nodeset.size == 1 && nodeset.first.is_a?(Nokogiri::XML::Element)
        break if nodeset.first.children.blank?

        nodeset = nodeset.first.children
        nodeset.each do |top_node|
          nodeset.delete(top_node) if top_node.is_a?(Nokogiri::XML::Text) && top_node.blank?
        end
      end
      nodeset.to_xhtml(indent_text: '', indent: 0).delete("\n")
    end
  end
end
