...
class PagesController < ApplicationController
  ...
  def reading
    ...
    @pages = Page.reading.recent.limit(@count + 5)[@count..]
  ...
  def soonest
    ...
    @count = params[:count].to_i
    @title = 'Read Next'
    @pages = Page.soonest.random.limit(@count + 5)[@count..]
    @new_query = { count: @count + Filter::LIMIT } if @pages.count == Filter::LIMIT
    flash.now[:alert] = 'No pages found' if @pages.blank?
  end

  def filter
    if params[:url]
      @page = Page.find_by(url: params[:url].normalize)
      if @page
    ...
    if params[:find]
      tag = Tag.with_short_name(params[:find])
  ...
  def find
    ...
    if page[:url]
      ...
      if normalized.blank? && page[:url].present?
        ...
        @page = Page.find_by(url: normalized)
      end
      if @page
  ...
  def update
    ...
    when 'Change'
      ...
      flash.now[:notice] = "Read set to #{@page.soon_label}"
    when 'Audiobook created'
      @page.make_audio
      flash.now[:notice] = 'Tagged with reader and marked as read today'
    when 'Increase Type'
      @page.increase_type
      flash.now[:notice] = 'Type increased'
    when 'Decrease Type'
      @page.decrease_type
      flash.now[:notice] = 'Type decreased'
    when 'Increase Position'
      @page.update position: @page.position + 1
      flash.now[:notice] = 'Position increased'
    when 'Decrease Position'
      @page.update position: @page.position - 1
      flash.now[:notice] = 'Position decreased'
    ...
    when 'Remove Downloads'
      ...
      flash.now[:notice] = 'Removed Downloads'
    when 'Rebuild Meta'
      @page.rebuild_meta
    ...
    when 'Make Me a Chapter'
      ...
      flash.now[:notice] = 'Made a chapter'
    when 'Make Single'
      @page.make_single
      flash.now[:notice] = 'Made Single'
    when 'Uncollect'
    ...
    when 'Edit HTML'
      ...
      flash.now[:notice] = 'Clean HTML updated.'
