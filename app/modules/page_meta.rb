...
module PageMeta
  def boring?(string)
    return true if string == PageTitles::TEMP_TITLE
    return true if string == 'temp' # old temp title

    %w[Part Chapter Page].each do |boring|
      matchdata = string.match(/^\d+\. (.*) \d+$/) || string.match(/^(.*) \d+$/)
      return false unless matchdata
      return true if boring == matchdata[1]
    end
    false
  end
  ...
  def end_notes_in_notes? = end_notes.present? && !at_end
  def show_notes = notes.present? || my_notes.present? || end_notes_in_notes?
  def show_end_notes = end_notes.present? && at_end

  def hr1? = notes.present? && my_notes.present?
  ...
  def book_doc
    if type == 'Book'
      Nokogiri::HTML(parts.first.raw_html)
    elsif type == 'Series'
      Nokogiri::HTML(navigate_html)
    else
      doc
  ...
  def set_wip?
    ...
    chapters = begin
      doc.css('.stats .chapters').children[1].text.split('/')
  ...
  def old_ff_style_hash
    ...
    begin
      create_hash book_doc.css('td script').text
    rescue StandardError
      {}
    end
  end
  ...
  def create_hash(string)
    ...
    array.each do |pair|
      ...
      key = key_value[0].delete_prefix('var ')
      key = key.to_sym
  ...
  def inferred_authors
    if can_have_tags?
      ...
      if ao3? || first_part_ao3?
        ...
        links = begin
          [doc.css('#profile_top a').first.text]
        rescue StandardError
          nil
        end
        [hash, links].pulverize
  ...
  def chapter_title
    if ao3?
      new =
        begin
          doc.css('.chapter .title').children.last.text.strip.gsub(/^: /, '')
        rescue StandardError
          nil
        end
      if boring?(title)
      ...
      new = begin
        doc.search('option[@selected="selected"]').children[0].text =~ /\d+\. (.*)/
        ::Regexp.last_match(1)
      rescue StandardError
        nil
      end
      old = begin
        doc.search('option[@selected="selected"]').children[1].text =~ /\d+\. (.*)/
        ::Regexp.last_match(1)
      rescue StandardError
        nil
      end
      ...
      doc.at('h1').text.tr('–', '—').split('—').second.squish
    elsif km?
  ...
  def work_title
    if ao3? || first_part_ao3?
      begin
        ...
        if title.blank? || title == PageTitles::TEMP_TITLE
          'title not found'
        else
          title
        end
      ...
      new = begin
        book_doc.css('#profile_top b').text
      rescue StandardError
        nil
      end
      ...
      doc.at('h1').text.tr('–', '—').split('—').first.squish
    elsif wikipedia?
  ...
  def inferred_title
    Rails.logger.debug { 'entering inferred_title' }
    Rails.logger.debug { "  for #{type} #{id} with current title: #{title}" }
    if type == 'Chapter'
      if title.blank? || boring?(title) || title == parent.title
        Rails.logger.debug { '  which is blank or boring or parent’s' }
        if chapter_title.blank? || boring?(chapter_title)
          Rails.logger.debug { ' as is the chapter title, setting boring default' }

          "Chapter #{position}"
        else
          Rails.logger.debug { ' but the chapter title is not: using it' }
          chapter_title
        end
      else
        Rails.logger.debug { '  which is neither blank nor boring: no change' }
        title
      end
    elsif type == 'Single'
  ...
  def work_summary
    Rails.logger.debug { "getting work summary for #{type} #{id}" }
    result = if wp?
               wp_try('Summary')
             elsif type == 'Series' && (ao3? || first_part_ao3?)
               dts = doc.css('.series dt')
               if dts.blank?
                 Rails.logger.debug { " couldn't find series info in navigate_html" }
               else
                 dds = doc.css('.series dd')
                 possibles = []
                 5.times do |i|
                   possibles << dds[i].css('blockquote').children.to_html if dts[i].text == 'Description:'
                 end
                 possibles.first
               end
               ...
               if ao3? || first_part_ao3?
                 ...
                 Rails.logger.debug { " don't know what to try" }
               end
             else
               Rails.logger.debug { 'not a work' }
             end
  ...
  def work_notes
    if wp? && type == 'Series'
      ...
      begin
        ...
        doc.css('.series dd')[4].css('blockquote').children.to_html if doc.css('.series dt')[4].text == 'Notes:'
      rescue StandardError
        ''
      end
    elsif %w[Book Single].include?(type)
  ...
  def chapter_summary
    return unless %w[Chapter Single].include?(type)
  ...
  def work_end_notes
    return unless %w[Book Single].include?(type)
  ...
  def note_tags = inferred_tags.without(PageTags::TT).without(PageTags::FI).join(', ')
  ...
  def head_notes(my_if, my_ia, my_ir)
    ...
    when 'Single'
      if chapter_as_single?
        ...
        [add_authors(my_ia), add_fandoms(my_if), my_ir.join(', '), work_summary, chapter_summary, note_tags,
         work_notes, chapter_notes]
      end
    when 'Book'
  ...
  def inferred_notes(my_if, my_ia, my_ir, force: false)
    if scrubbed_notes?
  ...
  def set_meta(force: false)
    unless ao3? || first_part_ao3? || ff? || first_part_ff? || wp? || wikipedia?
  ...
  def add_authors(strings)
    ...
    strings.each do |single|
      found = nil
      possibles = single.tr('(', ',').delete(')').split(',')
      possibles.each do |try|
        Rails.logger.debug { "trying '#{try}'" }
        found = Author.with_short_name(try.strip)
        next unless found
    ...
    if existing.present?
      ...
      existing.uniq.each { |a| tags << a unless tags.authors.include?(a) }
      update_tag_caches
    end
  ...
  def add_fandoms(strings)
    ...
    strings.each do |t|
      ...
      if found.blank? && simple.present?
        ...
        maybes = possibles.collect { |t| t unless trying.none? { |w| t.name.match(/\b#{w}\b/) } }
  ...
  # only used in Fandom.destroy_me and create_from_hash (test utility)
  def add_fandoms_to_notes(fandoms)
    update! notes: "#{add_fandoms(fandoms)}#{notes}"
    self
  end

  # only used in author.destroy_me
  def add_authors_to_notes(authors)
    update! notes: "#{add_authors(authors)}#{notes}"
    self
  end

  # only used in reader.destroy_me
  def add_reader_to_my_notes(reader)
    update! my_notes: "read by #{reader}<p>#{my_notes}"
    self
  end
  ...
  def wp_try(string)
    ...
    if found.blank?
      ...
      found = if try == "#{string}:"
                doc.css('strong').children.map(&:children).flatten.map(&:text).find { |t| t.match(string) }
              else
                try
              end
    end
    if found
      match = begin
        found.split(':', 2).second.squish
      rescue StandardError
        ''
      end
      return '' if (string == 'Warnings') && match.match?('None')

      match.gsub(%r{n/a}i, '').gsub(Regexp.new('</em><em>$'), '').gsub(Regexp.new('<br> ?$'), '').squish
    else
      ''
    end
  end
