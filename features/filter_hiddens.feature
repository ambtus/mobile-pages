Feature: pages with hidden tags are filtered out by default. During search, anything with a hidden is not found unless it is chosen from hiddens.
  ...
  Scenario: Find page by url SHOULD find hiddens
    Given the following pages
      | title                                              | url                                | hiddens |
      | A Christmas Carol by Charles Dickens               | http://test.sidrasue.com/cc.html   | hide me |
      | The Call of the Wild by Jack London                | http://test.sidrasue.com/cotw.html | |
      | The Mysterious Affair at Styles by Agatha Christie | http://test.sidrasue.com/maas.html | hide |
    When I am on the homepage
