...
Then('the temporary html file should exist') do
  assert File.exist?(Page.first.temporary_file('.html'))
end

Then('the temporary html file should NOT exist') do
  assert !File.exist?(Page.first.temporary_file('.html'))
end
Then('the temporary read file should exist') do
  assert File.exist?(Page.first.temporary_file('.read'))
end

Then('the temporary read file should NOT exist') do
  assert !File.exist?(Page.first.temporary_file('.read'))
end

Then('the temporary epub file should exist') do
  assert File.exist?(Page.first.temporary_file('.epub'))
end

Then('the temporary epub file should NOT exist') do
  assert !File.exist?(Page.first.temporary_file('.epub'))
end
...
Then(/^the download epub command for "([^"]*)" should include (.+): "([^"]*)"$/) do |title, option, text|
  ...
  epub_tags = Page.find_by(title: title).epub_tags
...
Then('the download epub title for {string} should be {string}') do |string, string2|
  epub_title = Page.find_by(title: string).epub_title
