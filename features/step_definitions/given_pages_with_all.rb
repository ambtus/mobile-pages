...
Given('pages with all possible wips exist') do
  ...
  Book.create!(title: 'book1', parent_id: series1.id, position: 1)
  Book.create!(title: 'book2', parent_id: series1.id, position: 2)
  series2 = Series.create!(title: 'wip series', wip: true)
  Book.create!(title: 'book1', parent_id: series2.id, position: 1)
  Book.create!(title: 'book2', parent_id: series2.id, position: 2)
  series3 = Series.create!(title: 'finished series with wip book')
  Book.create!(title: 'book1', parent_id: series3.id, position: 1)
  Book.create!(title: 'wip book2', parent_id: series3.id, position: 2, wip: true)
  series4 = Series.create!(title: 'wip series with wip book', wip: true)
  Book.create!(title: 'wip book1', parent_id: series4.id, position: 1, wip: true)
  Book.create!(title: 'book2', parent_id: series4.id, position: 2)
end
...
Given('pages with all possible types exist') do
  ...
  Chapter.create!(title: 'Epilogue', parent_id: book2.id, position: 1, url: 'http://test.sidrasue.com/parts/5.html')
...
Given('pages with all possible sizes exist') do
  Page.create(title: 'Medium', url: "file:///#{Rails.root.join('tmp/html/long.html')}")
  Page.create(title: 'Drabble').raw_html = Array.new(100) { |i| (i + 1).to_s }.join(' ')
  Page.create(title: 'Long2', base_url: "file:///#{Rails.root.join('tmp/html/long*.html')}", url_substitutions: '1-9')
  Page.create(title: 'Short', url: "file:///#{Rails.root.join('tmp/html/short.html')}")
  Page.create(title: 'Long', url: "file:///#{Rails.root.join('tmp/html/40000.html')}")
  Page.create(title: 'Epic', base_url: "file:///#{Rails.root.join('tmp/html/epic*.html')}", url_substitutions: '1-9')
  Page.create(title: 'Medium2', base_url: "file:///#{Rails.root.join('tmp/html/medium*.html')}",
url_substitutions: '1-5')
end
