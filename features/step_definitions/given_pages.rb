...
Given('link in notes exists') do
  ...
  page.notes = File.open(Rails.root.join('tmp/html/silent.html').to_s, 'r:utf-8', &:read)
  page.save!
end
...
Given('an editable page exists') do
  Page.create!(url: "file:///#{Rails.root.join('tmp/html/edit.html')}", title: 'Page 1')
end
...
Given('a work exists with chapter end_notes at end') do
  book = Book.create(title: 'Book')
  Chapter.create(title: 'ch1', parent_id: book.id, position: 1, end_notes: 'chapter 1 end notes',
url: 'http://test.sidrasue.com/test1.html', at_end: true)
  Chapter.create(title: 'ch2', parent_id: book.id, position: 2, end_notes: 'chapter 2 end notes',
url: 'http://test.sidrasue.com/test2.html', at_end: true)
  book.update_from_parts
end
...
Given('part {int} has reader {string}') do |int, string|
  ...
  part.add_tags_from_string(string, 'Reader')
end
